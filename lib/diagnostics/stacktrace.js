...
function stackStringToArray(stack, keepHeader, keepWhitespace) {
    if (keepHeader === void 0) { keepHeader = false; }
    if (keepWhitespace === void 0) { keepWhitespace = false; }
    var stackArray = stack.toString().split("\n");
    if (keepWhitespace !== true) {
        stackArray = stackArray.map(function (value) { return value.trim(); });
    }
    if (keepHeader !== true) {
        stackArray.shift();
    }
...
function getStackTrace(maxStackSize) {
    if (maxStackSize === void 0) { maxStackSize = 50; }
