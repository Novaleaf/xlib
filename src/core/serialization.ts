...
deserializes from a more relaxed superset of json (allows syntactically correct es5 javascript)
	...
	parse(
		...
		if ( typeName !== "string" ) {
			throw new ex.CorelibException( `JsonX.parse() failed due to typeof text != string  type=${ typeName }` );
		}
		var pojo: {};
		try {
			...
			if ( _.isObject( toReturn ) ) {
				...
				try {
					...
					_.forEach( node, ( value: any, key: string ) => {
						...
						if ( ( key in otherParams ) && ( otherParams[ key ] != null ) && ( _superStringifyTokenId in otherParams[ key ] ) ) {
