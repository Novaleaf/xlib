export declare type StripeStatic = (apiKey: string) => StripeInstance;
 ...
 * @param error
 */
export declare function generateUserDebugInfo(error: IError): {
    name: string;
    statusCode: number;
    type: string;
    param: any;
...
Returns the customer object if the update succeeded. Throws an error if update parameters are invalid (e.g. specifying an invalid coupon or an invalid source).*/
        update(stripeCustomerId: string, options: {
            /**optional
        ...
        deleteCard(customerId: string, cardId: string): Promise<{
            deleted: boolean;
            id: string;
        }>;
        /** Create a card
        ...
        createSource(customerId: string, options: {
            source: string;
        }): Promise<ICard | IAlipayAccount>;
        createSource(customerId: string, options: {
            source: string;
        }, /** optional A set of key/value pairs that you can attach to a card object. It can be useful for storing additional information about the card in a structured format. */ metadata: IMetadata): Promise<ICard | IAlipayAccount | IBitcoinReceiver>;
...
export {};
//# sourceMappingURL=stripe-d.d.ts.map